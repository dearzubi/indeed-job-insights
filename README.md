...
- **Driving time from your home city** shown on each card.
...
- **Home city** — e.g. `Mississauga, ON` or `London, UK`. This is the origin for driving-time calculations and the "home city match" detection.
...
- **Changing your home city or API key** takes effect on the next card view — no extension reload needed, though already-decorated cards keep their old driving times until the cache entry expires (30 days).
...
- Nothing leaves your machine except:
  - Requests to Google's Routes API with your home city and each listing's location (needed for driving time).
